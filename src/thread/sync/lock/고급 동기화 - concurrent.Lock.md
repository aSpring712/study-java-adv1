...
실행 결과를 보면 스레드가 `RUNNABLE` 상태로 깨어난 것을 확인할 수 있다. 그리고 해당 스레드의 인터럽트의 상태도
...
이 메서드들을 사용하면 고수준의 동기화 기법을 구현할 수 있다. `Lock` 인터페이스는 `synchronized` 블록보다
